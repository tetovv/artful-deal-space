...
import {
  ListVideo, History, Download, Plus, Trash2, MoreHorizontal,
  Eye, Clock, Play, Search, FolderOpen, GripVertical, X, Globe, Lock,
} from "lucide-react";
...
const Library = () => {
  ...
  const clearHistoryMutation = useMutation({
    ...
    onSuccess: (_, type) => {
      queryClient.invalidateQueries({ queryKey: [type === "view" ? "view-history" : "download-history"] });
      ...
      <div className="p-6 lg:p-8 space-y-6 max-w-5xl">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">Плейлисты, история просмотров и загрузок</p>
          </div>
        </div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList>
            <TabsTrigger value="playlists" className="gap-1.5">
              <ListVideo className="h-3.5 w-3.5" /> Плейлисты
            </TabsTrigger>
            <TabsTrigger value="history" className="gap-1.5">
              <History className="h-3.5 w-3.5" /> Просмотры
            </TabsTrigger>
            <TabsTrigger value="downloads" className="gap-1.5">
              <Download className="h-3.5 w-3.5" /> Загрузки
            </TabsTrigger>
          </TabsList>
              ...
              <Dialog open={createOpen} onOpenChange={setCreateOpen}>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <Button className="w-full" onClick={() => createPlaylistMutation.mutate()} disabled={!newPlaylist.title.trim() || createPlaylistMutation.isPending}>
                      Создать плейлист
                    </Button>
                  </div>
                </DialogContent>
                ...
                {filteredPlaylists.map((pl: any) => (
                  ...
                  >
                    <div className="aspect-video bg-muted relative flex items-center justify-center">
                      ...
                      <Badge className="absolute top-2 right-2 text-[9px] bg-black/60 text-white border-0">
                        {(playlistCounts as any)[pl.id] || 0} видео
                      </Badge>
                    ...
                    <div className="p-3 space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); deletePlaylistMutation.mutate(pl.id); }}
          ...
          <TabsContent value="history" className="space-y-4 mt-4">
            <div className="flex items-center justify-between">
              ...
              {viewHistory.length > 0 && (
                <Button variant="outline" size="sm" onClick={() => clearHistoryMutation.mutate("view")}>
                  <Trash2 className="h-3 w-3 mr-1.5" /> Очистить
                </Button>
              )}
            </div>
                ...
                {viewHistory.map((item: any) => (
                  ...
                  >
                    <div className="w-28 h-16 rounded-md overflow-hidden bg-muted shrink-0">
                      {item.content_items?.thumbnail ? (
                        <img src={item.content_items.thumbnail} alt="" className="w-full h-full object-cover" />
                      ) : (
                      ...
                      <div className="flex items-center gap-3 text-[11px] text-muted-foreground/60">
                        {item.watched_seconds > 0 && item.total_seconds > 0 && (
                          <span className="flex items-center gap-1">
                            ...
                            {Math.round((item.watched_seconds / item.total_seconds) * 100)}% просмотрено
                          </span>
                        )}
          ...
          <TabsContent value="downloads" className="space-y-4 mt-4">
            <div className="flex items-center justify-between">
              ...
              {downloadHistory.length > 0 && (
                <Button variant="outline" size="sm" onClick={() => clearHistoryMutation.mutate("download")}>
                  <Trash2 className="h-3 w-3 mr-1.5" /> Очистить
                </Button>
              )}
            </div>
